...
knitr::opts_knit$set(root.dir = '~/quant-methods')
```
...
First select subsets of variables that match regex patterns
```{r}
...
Then select those variables from the full data frames
```{r}
...
Define two_sim function to get the cosine similarity of two religions based on a given similarity matrix
```{r}
...
Compare Presbyterian Churches using two_sim
```{r}
